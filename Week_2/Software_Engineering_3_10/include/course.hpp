...
class Course {
 private:
  ...
  std::vector<double> studentGrades_;  // Vector to store student grades

 public:
  Course(const std::string& courseName,
         const int& totalStudents);  // Default constructor
  ~Course();                         // Default destructor

  double getStudentGrade(const int& studentNo);
  void updateStudentGrade(const int& studentNo, const double& newGrade);
  double calcCourseAvg();
  void displayCourseSummary();
};

#endif  // WEEK_2_SOFTWARE_ENGINEERING_3_10_INCLUDE_COURSE_HPP_
